    ...
    <div class="w-3/6 border-black border-r border-l">
      ...
      <div class="h-1/2 border-black border-t flex flex-col">
        {#if state.currentRoute}
          ...
          <ActionButton label="Date" classes="bg-green-500 text-white" onclick={() => (state.setAtPopup = true)}>
          ...
          >
            {#snippet icon()}<Icon name="tools" color={route.toRemove ? '#f59e0b' : '#fff'} size="30px" />{/snippet}
          </ActionButton>

          <ActionButton
            label="À ouvrir"
            classes="border border-purple-600 {route.toOpen ? 'bg-white text-purple-600' : 'bg-purple-600 text-white'}"
          ...
          <ActionButton label="Supprimer" classes="bg-red-500 text-white" onclick={() => state.deleteCurrent()}>
