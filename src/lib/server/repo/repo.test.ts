...
describe('clubs supprimés', () => {
  beforeEach(async () => {
    await db.updateTable('club').set({deletedAt: new Date().toISOString()}).where('id', '=', club.id).execute()
  })
...
describe('suppression logique', () => {
  ...
  const deletionDate = async (id: string) =>
    (await db.selectFrom('route').select('deletedAt').where('id', '=', id).executeTakeFirst())?.deletedAt
