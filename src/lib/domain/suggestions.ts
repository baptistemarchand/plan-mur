...
export const getSuggestions = (lines: Route[][]): Suggestion[] => {
  const isAvailable = (color: Color, lineIndex: number): boolean => {
    ...
    return lines[lineIndex].every(route => route.color !== color || route.toRemove || !isLive(route))
  }
  ...
  const canSet = (color: Color, lineIndex: number) => {
    ...
    return isAvailable(color, lineIndex) && isAvailable(color, lineIndex - 1) && isAvailable(color, lineIndex + 1)
  }
