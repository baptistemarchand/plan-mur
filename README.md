...
Gestion des voies de murs d'escalade, pour deux clubs.

SvelteKit sur Cloudflare Workers, base D1. Remplace l'app Fresh / Deno KV qui
occupait ce dépôt jusqu'ici ; son historique reste dans git, et `migration/`
garde de quoi exporter la base KV tant que la bascule n'est pas faite.
...
La production tourne encore sur Deno KV. Le jour de la bascule, réexporter :
son contenu aura bougé depuis le dump de septembre.

```
cd migration && deno task export-kv    # dump brut, voir migration/README.md
cd .. && npm run db:import export/kv-<date>.json
npx wrangler d1 execute plan-mur --local --file=seeds/import.sql
```
